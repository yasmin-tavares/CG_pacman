/* -------------------------------------------------------------- CRIAÇÃO DO MAPA ---------------------------------------------------- */
var criar_mapa = function (cena, proporcao_labirinto) {
  // função que cria o mapa recebe como parâmetro a scene e a definição do nível(?)
  ...
  var x, y;
  for (var linha = 0; linha < proporcao_labirinto.length; linha++) {
    // delimitando as coordenadas do mapa
    y = -linha;
    mapa[y] = {};
    ...
    for (
      var coluna = 0;
      coluna < proporcao_labirinto[linha].length;
      coluna += 2
    ) {
      // Salta cada segundo elemento, que é apenas um espaço para legibilidade.
      x = Math.floor(coluna / 2);
      var celula = proporcao_labirinto[linha][coluna];
      var object = null;

      if (celula === "#") {
        // aqui começa a criação de uma parede com a chamada de função da mesma
        object = criar_parede(); // """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      } else if (celula === ".") {
        // aqui começa a criação das pílulas com a chamada de função da mesma
        object = criar_pilulas(); // """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""
        mapa.pilulas_comuns += 1; // """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      } else if (celula === "o") {
        // define o local das pílulas especiais
        object = criar_pilulas_especiais(); // """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      } else if (celula === "P") {
        // define o local de início do pacman
        mapa.pacmanSpawn = new THREE.Vector3(x, y, 0); // """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      } else if (celula === "G") {
        // define o local de inicio dos fantasmas
        mapa.fantasmaSpawn = new THREE.Vector3(x, y, 0); // """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""
      }
      if (celula === "D") {
        // define o local de inicio da camera
        d1 = [y, x, 0]; // """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""
        console.log(d1);
      }
