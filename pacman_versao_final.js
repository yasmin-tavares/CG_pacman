	...
	var score = 0;
	var lives;
	var defaultCamera = [0, 0, 0];
	var pauseOrbit;
	var camera_player = true;
	var runAnim = true;
    var LEVEL = [
        '# # # # # # # # # # # # # # # # # # # # # # # # # # # #',
        '# . . . . . . . . . . . . # # . . . . . . . . . . . . #',
  ...
  var criar_mapa = function (cena, proporcao_labirinto) { 												// função que cria o mapa recebe como parâmetro a scene e a definição do nível(?)
        ...
        for (var linha = 0; linha <proporcao_labirinto.length; linha++) {								// delimitando as coordenadas do mapa
            ...
            for (var coluna = 0; coluna < proporcao_labirinto[linha].length; coluna += 2) { 			// Salta cada segundo elemento, que é apenas um espaço para legibilidade.
				...
				if (celula === 'D') {															// define o local de inicio da camera
                    d1 = [y, x, 0];																		// """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""
					console.log(d1);
                }
	...
	function togglePause(cam)
{
	...
	var main = function () {
        ...
        var camera = new THREE.PerspectiveCamera(65, window.innerWidth / window.innerHeight, 0.1, 100000);
        camera.up.copy(UP);
	...
	/* ---------------------------------------  UPDATES ----------------------------------*/
        var update = function (delta, now) {
            ...
            cena.children.forEach(function (object) {
                if (object.isfantasma === true)
                    updatefantasma(object, delta, now);
                if (object.isWrapper === true)
                    wrapObject(object, mapa);
        ...
        var movePacman = function (delta) {
            ...
            var esquerdaSide = pacman.position.clone().addScaledVector(esquerda, PACMAN_RADIUS).round();
            var topoSide = pacman.position.clone().addScaledVector(topo, PACMAN_RADIUS).round();
            var direitaSide = pacman.position.clone().addScaledVector(direita, PACMAN_RADIUS).round();
            var baixoSide = pacman.position.clone().addScaledVector(baixo, PACMAN_RADIUS).round();
        ...
        var updatefantasma = function (fantasma, delta, now) {
            ...
            if (!lost && !won && distance(pacman, fantasma) < PACMAN_RADIUS + fantasma_RADIUS) {
                if (fantasma.isAfraid === true) {
					...
					if (lives <= 0){
						//death();
						window.alert('fim de jogo; pontuacao: ' + score + 'recarregue a pagina para tentar novamente;');
					}
                }
            }
        }
        ...
        var movefantasma = function () {
            ...
            return function (fantasma, delta) {
                previousPosition.copy(fantasma.position).addScaledVector(fantasma.direction, 0.5).round();
                fantasma.translateOnAxis(fantasma.direction, delta * fantasma_SPEED);
                currentPosition.copy(fantasma.position).addScaledVector(fantasma.direction, 0.5).round();
                ...
                if (!currentPosition.equals(previousPosition)) {
                    ...
                    var esquerdaparede = parede(mapa, currentPosition.copy(fantasma.position).add(esquerdaTurn));
                    var direitaparede = parede(mapa, currentPosition.copy(fantasma.position).add(direitaTurn));
